...
const suggestion = (label: string) => (
  <span className="inline-block px-3 py-1 bg-orange-100 dark:bg-gray-700 text-orange-700 dark:text-orange-300 rounded-full text-xs mr-2 mb-2 cursor-pointer">
    {label}
  </span>
);
...
const OnboardingChat: React.FC = () => {
  ...
  const handleSend = (val: string) => {
    ...
    if (next) {
      ...
      // Done: persist data and redirect
      ...
      setMessages(m => [...m, { id: `d${m.length}`, role: 'bot', text: 'Thanks! Your profile is ready. Redirecting to your dashboard...' }]);
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-[#1a2035] text-gray-900 dark:text-white transition-colors duration-200">
      <div className="max-w-3xl mx-auto px-4 py-6">
        <div className="flex items-center justify-between mb-4">
          <h1 className="text-2xl font-bold">Tell us about you</h1>
          <ThemeToggle />
        </div>
        <div ref={listRef} className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-4 h-[65vh] overflow-y-auto">
          {messages.map(msg => (
        ...
        >
          <input
            ...
            className="flex-1 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-lg px-3 py-2 outline-none focus:ring-2 focus:ring-orange-500 text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400"
          />
          <button type="submit" className="bg-orange-500 hover:bg-orange-600 text-white px-4 py-2 rounded-lg">Send</button>
        </form>
      </div>
    </div>
  );
